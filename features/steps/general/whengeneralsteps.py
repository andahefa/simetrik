...
@when(u'I search "(?P<page>.*)" in the input')
def step_impl(context, page):
    return context.current_page.text_value_in_the_filter(page)


@when(u'I click on the "(?P<option>.*)" option')
def step_option(context, option):
    home_page = context.home
    home_page.click_option(option)
